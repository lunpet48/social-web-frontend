...
const MediaSlider = ({
  ...
  const setSliderDimensions = async () => {
    ...
    if (sliderRef.current) {
      const width = await calculateSliderWidth(fixedWidth, fixedHeight, urlFiles);
      const height = await calculateSliderHeight(fixedWidth, fixedHeight, urlFiles);
      ...
      {files?.map((file, index) => {
        if (file.type.startsWith('image')) {
          ...
          return (
            ...
            >
              <VideoPlayerComponent src={URL.createObjectURL(file)} />
            </div>
          );
        }
      })}
